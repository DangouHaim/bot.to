...
class Similar_Listings extends Query {
    ...
    public function run( $listing_id ) {
        ...
        if ( $settings['orderby'] === 'proximity' && ( $latitude = $listing->get_data( 'geolocation_lat' ) ) && ( $longitude = $listing->get_data( 'geolocation_long' ) ) ) {
            ...
            $sql = $wpdb->prepare( \MyListing\Src\Queries\Explore_Listings::instance()->get_proximity_sql(), $earth_radius, $latitude, $longitude, $latitude, $settings['max_proximity'] );
            ...
            add_filter( 'mylisting/preview-card/show-badge', '__return_false', 55 );
        }
