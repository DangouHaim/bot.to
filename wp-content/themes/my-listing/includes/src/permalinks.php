...
class Permalinks {
    ...
    public function get_listing_types( $refresh = false ) {
        ...
        while( $types->have_posts() ) {
            $types->the_post();
            if ( $type = \MyListing\Ext\Listing_Types\Listing_Type::get( $post->ID ) ) {
    ...
    public static function get_permalink_structure() {
        ...
        /**
         * For WPJM versions 1.32.0 and higher, permalink settings were stored in
         * `job_manager_permalinks` option, stored as string using `wp_json_encode`.
         * Maintain backward compatibility while the theme ditches WPJM.
         *
         * @since 2.1.0
         * @deprecated 2.2.0
         */
        $job_manager_permalinks = json_decode( get_option( 'job_manager_permalinks' ), true );
        if ( json_last_error() === JSON_ERROR_NONE && is_array( $job_manager_permalinks ) ) {
            $permalinks = array_merge( $permalinks, $job_manager_permalinks );
        }

        /**
         * For WPJM versions 1.31.3 and lower, permalink settings were stored in
         * `wpjm_permalinks` option, in serialized format.
         * Maintain backward compatibility while the theme ditches WPJM.
         *
         * @since 2.1.0
         * @deprecated 2.2.0
         */
        $wpjm_permalinks = get_option( 'wpjm_permalinks' );
        if ( is_array( $wpjm_permalinks ) && ! empty( $wpjm_permalinks ) ) {
            $permalinks = array_merge( $permalinks, $wpjm_permalinks );
        }

        /**
         * Since MyListing 2.1.0, permalink settings are stored in `mylisting_permalinks`
         * setting, in serialized format. This is necessary since `wpjm_permalinks` option
         * now gets deleted by WPJM in new versions.
         */
