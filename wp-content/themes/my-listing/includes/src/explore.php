...
class Explore {
	...
	public function __construct( $data ) {
		...
		$this->types = array_filter( array_map( function( $listing_type ) {
			return \MyListing\Ext\Listing_Types\Listing_Type::get_by_name( $listing_type );
	...
	public function get_types_config( $filters = [] ) {
		$config = [];
		$index = 0;
		foreach ( (array) $this->types as $type ) {
			...
			$config[ $type->get_slug() ] = [
				...
				'id' => $type->get_id(),
				'tabs' => $tabs,
				'filters' => isset( $filters[ $type->get_slug() ] ) ? $filters[ $type->get_slug() ] : new \stdClass,
				'tab' => $active_tab,
