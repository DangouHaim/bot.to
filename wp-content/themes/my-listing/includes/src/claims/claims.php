...
class Claims {
	...
	public function __construct() {
		...
		// Claimed body class.
		add_filter( 'body_class', [ $this, 'claimed_body_classes' ] );
	...
	/**
	 * Add a CSS class to claimed listings
	 *
	 * @since 1.6
	 */
	public function claimed_body_classes( $classes ) {
		if ( is_singular( 'job_listing' ) ) {
			$listing = \MyListing\Src\Listing::get( get_queried_object_id() );
	...
	public static function get_claim_url( $listing_id ) {
		...
		// make sure listing isn't already claimed, and the claimer isn't the listing author
		if ( $listing->is_claimed() || absint( get_current_user_id() ) === absint( $listing->get_author_id() ) ) {
			return '';
		}
