...
class Promotions {
	...
	public function handle_promotion_request() {
		...
		if ( $process === 'cancel-package' ) {
			try {
				...
				wc_add_notice( sprintf( _x( 'Promotion cancelled for "%s".', 'Promotions: buy package', 'my-listing' ), $listing->post_title ), 'notice' );
	...
	public function expire_package( $package_id ) {
		...
		// mlog( sprintf( 'Listing %d with old priority: %d', $listing_id, get_post_meta( $listing_id, '_promo_package_old_priority', true ) ) );
		...
		// Trash package.
		remove_action( 'save_post', [ Package::instance(), 'save_package' ], 99 );
		wp_trash_post( $package_id );
		add_action( 'save_post', [ Package::instance(), 'save_package' ], 99, 2 );
	...
	public function flush_listings_cache() {
		mlog( '[Promotions] Updating cache version for listings.' );
	...
	public function listing_feed_args( $args, $data ) {
		if ( empty( $data['priority_levels'] ) || count( $data['priority_levels'] ) === 4 ) {
			// all priority levels are included
			return $args;
		}

		$levels = [];
		if ( in_array( 'normal', $data['priority_levels'] ) ) { $levels[] = 0; }
		if ( in_array( 'featured', $data['priority_levels'] ) ) { $levels[] = 1; }
		if ( in_array( 'promoted', $data['priority_levels'] ) ) { $levels[] = 2; }
		if ( in_array( 'custom', $data['priority_levels'] ) ) { $levels[] = 3; }

		// filter by priority
		$args['meta_query']['listing_feed_priorities_clause'] = [
			'relation' => 'OR',
			[
				'key' => '_featured',
				'value' => $levels,
		...
		// if it's set to filter normal priorities, handle listings without the priority meta key set
		if ( in_array( 'normal', $data['priority_levels'] ) ) {
			$args['meta_query']['listing_feed_priorities_clause'][] = [
				'key' => '_featured',
				'compare' => 'NOT EXISTS',
			];
		...
		// if it's set to filter custom priorities, this includes all priorities higher than 2, so add a special rule for that case
		if ( in_array( 'custom', $data['priority_levels'] ) ) {
			$args['meta_query']['listing_feed_priorities_clause'][] = [
				'key' => '_featured',
				'value' => '2',
				'compare' => '>',
