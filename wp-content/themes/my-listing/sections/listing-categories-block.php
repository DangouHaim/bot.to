...
if ( ! is_array( $data['terms'] ) || ! array_filter( $data['terms'] ) || is_wp_error( $data['terms'] ) ) {
	return false;
}
?>
...
<div class="<?php echo esc_attr( $data['wrapper_class'] ) ?>" <?php echo $data['wrapper_id'] ? sprintf( 'id="%s"', $data['wrapper_id'] ) : '' ?>>
	<div class="element categories-block">
		...
		<div class="pf-body">
			<div class="listing-details <?php echo esc_attr( sprintf( 'item-count-%s', count( (array) $data['terms'] ) ) ) ?>">
				<ul>
