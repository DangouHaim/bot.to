<?php
	...
	foreach ((array) explode(',', (string) $data['listing_types']) as $listing_type) {
		...
		if ( $listing_type ) {
			$listing_types[] = new MyListing\Ext\Listing_Types\Listing_Type( $listing_type[0] );
		}
	}
?>
	...
	<div class="featured-search <?php echo esc_attr( $data['layout'] ) ?>">
		<div class="fs-tabs">
			...
			<div class="tab-content <?php echo $data['box_shadow'] == 'yes' ? 'add-box-shadow' : '' ?>">
				...
				foreach ($listing_types as $type): $i++;
					$filters = [];
					foreach ( (array) $type->get_search_filters( 'basic' ) as $facet ) {
						ob_start();
						mylisting_locate_template( sprintf( 'partials/facets/%s.php', $facet['type'] ), [
							'facet' => $facet,
